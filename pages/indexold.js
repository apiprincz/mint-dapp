...
export default function Home() {
  ...
  const getProof = async () => {
    ...
    console.log("mintPrice", "mintProof", rootHash);
  };
  ...
  const checkTotalFreeMinted = async () => {
    try {
      ...
      const totalFreeMinted = (
        await bunniesContract.totalFreeMinted()
      ).toNumber();
  ...
  const callWhitelistMint = async () => {
    try {
      ...
      console.log("mintPrice", "mint", Number(mintPrice), rootHash);
      setDisplayPrice(Number(mintPrice));

      const tx = await bunniesContract.mint(quantity, merkleProof, {
        value: mintPrice,
      });
      console.log("mintPrice", "mint", mintPrice, rootHash);
      setLoading(true);
      await tx.wait();
      setLoading(false);
  ...
  const callPublicMint = async () => {
    try {
      ...
      const mintPrice = utils.parseEther("0.007").mul(quantity);
      const tx = await bunniesContract.publicMint(quantity, {
        value: mintPrice,
      });
      setLoading(true);
      await tx.wait();
      ...
      if (merkleProof.length > 0) {
        return (
          ...
          <div>
            ...
            {}
            <button className={styles.mint_button} onClick={callWhitelistMint}>
              Mint
            </button>
          </div>
        ...
        return (
          <div className={styles.noWL}>
            ...
            <br />
            <br />
            You can get one on <a href="#" className={styles.colored} >opensea</a>
    ...
    if (publicMintStage) {
      return (
        <div>
          ...
          <button className={styles.mint_button} onClick={callPublicMint}>
            Mint
          </button>
        </div>
      );
  ...
  function renderMintStatus() {
    if (whitelistMintStage) {
      return <span className={styles.stage}>WHITELIST MINT</span>;
    } else if (publicMintStage) {
      return <span className={styles.stage}>PUBLIC MINT</span>;
    } else {
